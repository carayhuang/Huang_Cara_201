...
cat("The largest win spread is", spread[max_spread_index], ", when the Bears scored", bears[max_spread_index], "points and the Opponent scored", opponent[max_spread_index], "points.\n")
```
...
rectangle_area <- function(length = 48, width = 48) {
  ...
  return(c(area_sq_in, area_sq_ft))
}
result <- rectangle_area(102, 60)
cat("The area in square inches is", result[1], "\n")
cat("The area in square feet is", result[2], "\n")
```
...
```{r}
convert_negatives_to_na <- function(mat) {
  mat[mat < 0] <- NA
  return(mat)
}
print(convert_negatives_to_na(x))
```
